...
from kivy.uix.widget import Widget  # لتعبئة المساحة المتبقية ودفع العناصر للأعلى
...
class ScreenChartSelect(Screen):
    def __init__(self, **kwargs):
        ...
        # إضافة عنصر فارغ لامتصاص الفراغ من الأسفل ودفع الأزرار للأعلى
...
class ScreenDimensions(Screen):
    def __init__(self, **kwargs):
        ...
        # إضافة عنصر فارغ لدفع المدخلات إلى أعلى الشاشة
...
class ScreenResult(Screen):
    ...
    def calculate_and_display(self):
        ...
        # الحسابات الإحصائية بحسب المخطط
        ...
        # تحديد النقاط الخارجة عن السيطرة
        self.ooc_list = []
        for idx, val in zip(indices, values):
            if val > ucl or val < lcl:
                self.ooc_list.append((idx, val))

        # رسم المخطط باستخدام Matplotlib بخلفية بيضاء
        fig, ax = plt.subplots(figsize=(7, 4))
        fig.patch.set_facecolor('white')
        ax.set_facecolor('white')
        
        # رسم الخطوط العادية والنقاط
        ax.plot(indices, values, color='blue', marker='o', linestyle='-', label='Sample Values')
        
        # إبراز النقاط الخارجة عن السيطرة باللون الأحمر
        if self.ooc_list:
        ...
        # رسم حد السيطرة والحدود العليا والدنيا
        ...
        # حفظ الصورة مؤقتاً وعرضها في Kivy
    ...
    def remove_ooc_samples(self, instance):
        ...
        # تصفية البيانات وحذف العينات غير المنضبطة
        new_data = []
        new_indices = []
